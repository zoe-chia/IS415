...
```{r}
#vacc_jkt_ts <- spacetime(vaccinations_jkt,
                         jkt,
                         .loc_col = "SUBDISTRICT",
                         .time_col = "Date")
```
...
```{r}
#is_spacetime_cube(vacc_jkt_ts)
```

```{r}
#vacc_jkt_nb <- vacc_jkt_ts %>%
#  activate("geometry") %>%
...
#                                    alpha = 1))
#  HCSA <- wm_idw %>%
#    mutate(local_gi = localgstar_perm(
#      Dose1_Rate,
#      nb,
#      wt,
#      nsim = 99
...
#    Vaccination_Rate, nb, wts, nsim = 99),
#         .before = 1) %>%
#  unnest(local_Gi)
```
...
```{r}
# significant_HCSA <- HCSA %>% filter(p_sim < 0.05)
```
...
There seems to be a consistent cluster at the southwest and central regions of Jakarta. The cool clusters at the north of Jakarta also seem to be consistent. The government may want to focus and increase vaccination rates in those areas.
:::
